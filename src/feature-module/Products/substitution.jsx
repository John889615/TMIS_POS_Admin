...
const SubstitutionPage = () => {
    ...
    const fetchRecords = async () => {
        try {
            if (id) {
                const data = await getAllSubstitutionById(id);
                setListData(data);
            }
    ...
    const handleAdd = async (data) => {
        try {
            if (data.ProductSubstitutionID) {
                await updateSubstitution(data);
            }
            else {
                await newSubstitution(data);
            }
            await fetchRecords();
            setModelShow(false);
        } catch (err) {
                ...
                <div className="page-header">
                    <div className="add-item d-flex">
                        <div className="page-title">
                            <h4>Substitutions</h4>
                            <h6>Manage Your Substitution</h6>
                        </div>
                    </div>
                                    ...
                                    {filteredData.length > 0 ? (
                                        filteredData.map((item, index) => (
                                            <tr key={index}>
                                                ...
                                                <td>{item.ProductSubstitute || "N/A"}</td>
