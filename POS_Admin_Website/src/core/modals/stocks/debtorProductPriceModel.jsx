...
const DebtorProductPriceForm = ({
  ...
  const isEdit = useMemo(() => !!data?.POS_DebtorProductPriceID, [data]);

  // ✅ Only reset when adding new, not when editing
  useEffect(() => {
    if (showModel && formRef.current && !isEdit) {
  ...
  const handleSubmit = (e) => {
    ...
    const payload = {
      // ✅ required FK
      FK_DebtorProductID: id ? parseInt(id, 10) : null,

      FK_PriceCodeID: form.FK_PriceCodeID.value ? parseInt(form.FK_PriceCodeID.value, 10) : null,
      FK_TaxID: form.FK_TaxID.value ? parseInt(form.FK_TaxID.value, 10) : null,

      // ✅ YOU WERE MISSING THIS
      ItemPrice: form.ItemPrice.value ? parseFloat(form.ItemPrice.value) : null,

      Vat: form.Vat?.value ? parseFloat(form.Vat.value) : null,
    ...
    if (data?.POS_DebtorProductPriceID) {
      payload.POS_DebtorProductPriceID = data.POS_DebtorProductPriceID;
    }
    ...
    <Modal show={showModel} onHide={handleClose} centered dialogClassName="custom-modal-two">
      <form onSubmit={handleSubmit} ref={formRef}>
        <Modal.Header closeButton className="custom-modal-header border-0">
          <Modal.Title>Debtor Product Price</Modal.Title>
        </Modal.Header>
            ...
            <div className="col-lg-6">
              <div className="input-blocks">
                ...
                <select
                  ...
                  defaultValue={data?.FK_PriceCodeID ?? ""}
                >
            ...
            <div className="col-lg-6">
              <div className="input-blocks">
                ...
                <input
                  ...
                  type="datetime-local"
                  required
                  defaultValue={toDateTimeLocalValue(data?.StartDate)}
                  className="form-control"
                />
              </div>
            </div>
            ...
            <div className="col-lg-6">
              <div className="input-blocks">
                ...
                <input
                  ...
                  type="datetime-local"
                  required
                  defaultValue={toDateTimeLocalValue(data?.EndDate)}
                  className="form-control"
                />
              </div>
            </div>
        ...
        <Modal.Footer className="modal-footer-btn d-flex justify-content-end">
          ...
          <button type="submit" className="btn btn-submit">
            {data?.POS_DebtorProductPriceID ? "Update" : "Add"}
          </button>
        </Modal.Footer>
      </form>
    </Modal>
