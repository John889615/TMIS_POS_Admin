...
namespace POS_Webservice.Controllers
{
    ...
    public class SyncController : ControllerBase
    {
        ...
        // 2026-05-06 (Spec 2): Removed three legacy pull-from-server GET endpoints
        //   GET list/bookings, GET list/guests, GET list/booking/guests
        // Their service implementations were commented out previously and have
        // never been wired up. Re-add the controllers when the impls return.
        ...
        #region To Server (Spec 2 - unified push batch endpoint)

        /// <summary>
        /// POST /api/sync/push/batch
        /// Unified FOH-to-Admin push endpoint. FOH calls this once per group
        ...
        public async Task<IActionResult> PushBatch_Async([FromBody] Req_Push_Batch request)
        {
            if (!ModelState.IsValid)
            {
                ...
                _logger.LogValidation(ControllerLoggerExtensions.Format_ControllerInfo(this, "PushBatch validation failed", errors));
                return BadRequest(ApiResponse.Fail<Req_Push_Batch>(AppErrorCode.ValidationError, errors));
            }

            ApiResponse<Res_Push_Batch> result;
            try
            {
                result = await _syncService.Process_Push_Batch(request);
            }
            catch (Exception ex)
            {
                _logger.LogController(ControllerLoggerExtensions.Format_ControllerInfo(this, "PushBatch unhandled exception", ex));
                result = ApiResponse.Fail<Res_Push_Batch>(AppErrorCode.ServerError, new List<string> { ex.Message }, 500);
            }

            if (result == null)
            {
                _logger.LogController(ControllerLoggerExtensions.Format_ControllerInfo<object>(this, "PushBatch null response"));
                return StatusCode(500, ApiResponse.Fail<object>(AppErrorCode.UnknownError, new List<string> { "response was null" }, 500));
            }

            if (!result.Success)
            {
                _logger.LogController(ControllerLoggerExtensions.Format_ControllerInfo(this, "PushBatch failed", result));
                return StatusCode(result.StatusCode ?? 400, result);
            }

            _logger.LogController(ControllerLoggerExtensions.Format_ControllerInfo(this, "PushBatch succeeded", new
            {
                request?.BatchID,
                request?.Group,
                EntityCount = result.Data?.Results?.Count
            }));
            return Ok(result);
        }
        #endregion
